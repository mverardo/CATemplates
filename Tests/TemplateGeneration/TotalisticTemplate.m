...
With[
  ...
  Print[TotalisticTemplate[2, 2.0] === expectedTemplate]
];
...
With[
  ...
  Print[TotalisticTemplate[3, 1.0] === expectedTemplate]
];

Print["OuterTotalisticTemplate"];

With[
  {expectedTemplate = <|"k" -> 2, "r" -> 1.0, "rawList" -> {x7, x3, x5, x1, x3, x2, x1, x0}, "expansionFunction" -> RawExpansion |>},
  Print[OuterTotalisticTemplate[2, 1.0] === expectedTemplate]
];
...
With[
  ...
  Print[OuterTotalisticTemplate[2, 2.0] === expectedTemplate]
];
...
With[
  ...
  Print[OuterTotalisticTemplate[3, 1.0] === expectedTemplate]
];