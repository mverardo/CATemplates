...
BaseTemplate::usage = 
  "Gives the base template for a radius r k-ary rule.";

ValidTemplateQ::usage = "Determines if a template has avalid form.";

RuleTemplateVars::usage = 
  "Extracts the variable names used in a rule template and returns \
them in a list. The names are given in lexicographical order; for \
instance, RuleTemplateVars[MaxSymmTemplate[{BWLR, BW, LR}, 2, 1]] \
returns {x2, x1, x0}. If the template is in the \!\(\*
StyleBox[\"k\",\nFontSlant->\"Italic\"]\)-ary form, the function \
returns {}.";

TakeNeighbourhoods::usage = 
  "Returns the n first neighborhoods from a given space.";

TemplateFromNeighbourhoods::usage = 
  "Builds a template given a list of neighbourhoods. Converts the \
neighbourhoods to symbols in the form xN, where N is the decimal \
conversion of the k-ary neighbourhood.";

TemplateVarFromNeighbourhood::usage = 
  "Returns the template symbol orresponding to a given neighbourhood";

NeighborhoodFromTemplateVar::usage = 
  "Returns the neighbourhood corresponding to a given template symbol";

BWTransform::usage = 
  "Performs the Black-White transform in a rule table, transition, \
neighbourhood or transition output.";

LRTransform::usage = 
  "Performs the Left-Right transform in a rule table, transition, \
neighbourhood or transition output.";

BWLRTransform::usage = 
  "Performs the Black-White transform followed by the Left-Right \
transform in a rule table, transition, neighbourhood or transition \
output.";

LRBWTransform::usage = 
  "Performs the Left-Right transform followed by the Black-White \
transform in a rule table, transition, neighbourhood or transition \
output.";
...
ValidTemplateQ[template_] :=
  ...
  And @@ (MatchQ[#, (_Symbol | _Integer | _Plus | _Times | _ \
\[Element] {__})] & /@ template);
...
TakeNeighbourhoods[n_Integer, k_Integer: 2, r_Integer: 1] :=
 ...
 With[{neighbourhoodCount = k^(2 r + 1)},
  If[n <= neighbourhoodCount,
   IntegerDigits[#, k, \[LeftFloor]2 r + 1\[RightFloor]] & /@ 
    Range[neighbourhoodCount - 1, (neighbourhoodCount - n), -1],
...
NeighborhoodFromTemplateVar[templateVar_Symbol, k_Integer: 2, 
  r_Integer: 1] :=
 
 IntegerDigits[FromDigits[StringDrop[ToString@templateVar, 1]], k, 
  2*r + 1]
...
BWTransform[parameter_] :=
 (*Se o parametro for a rule table inteira*)
  ...
  (*Mapeio a pr\[OAcute]pria transforma\[CCedilla]\[ATilde]o em cada transi\[CCedilla]\[ATilde]o, 
  fa\[CCedilla]o um Sort pela vizinhan\[CCedilla]a e inverto a ordem*)
  
  Reverse[SortBy[BWTransform /@ parameter, First]],
  (*Faz a transforma\[CCedilla]\[ATilde]o em vizinhan\[CCedilla]as, outputs, 
  ou transi\[CCedilla]\[OTilde]es no formato {{vizinhan\[CCedilla]a}, output}*)
  1 - parameter
  ]
